        ...
        <div class="container-fluid">
            ...
            @if($activeFilter)
                <div class="row">
                        ...
                        <div class="col-md-4 no-print">
                            <div class="card card-info card-outline shadow-sm">
                                <div class="card-header">
                                    <h3 class="card-title">Add Slot: <strong>{{ $selectedEntity->name }}</strong></h3>
                                </div>
                                <form action="{{ route('timetable.store') }}" method="POST">
                                    ...
                                    <div class="card-footer">
                                        <button type="submit" class="btn btn-info btn-block"><i class="fas fa-plus"></i> Add to Timetable</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    ...
                    <div class="{{ $activeFilter === 'section' ? 'col-md-8' : 'col-md-12' }}">
                        <div class="card shadow-sm printable-area">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h3 class="card-title font-weight-bold">
                                    @if($activeFilter === 'section')
                                        Timetable for: {{ $selectedEntity->classLevel->name }} - {{ $selectedEntity->name }}
                                    @else
                                        Schedule for: {{ $selectedEntity->name }} (Teacher)
                                    @endif
                                </h3>
                                <div class="card-tools no-print">
                                    ...
                                    <a href="{{ route('timetable.index') }}" class="btn btn-sm btn-default ml-1">Clear</a>
                                </div>
                            </div>
                                                    ...
                                                    <div class="d-flex flex-wrap">
                                                        @if(isset($timetableGrid[$day]))
                                                            @foreach($timetableGrid[$day] as $slot)
                                                                <div class="border rounded p-2 m-1 bg-white shadow-sm" style="min-width: 160px; border-left: 4px solid #007bff !important;">
                                                                    <strong class="d-block text-primary">{{ $slot->subject->name }}</strong>
                                                                    ...
                                                                    <small class="d-block mt-1 font-weight-bold">
                                                                        @if($activeFilter === 'section')
                                                                            <i class="fas fa-user-tie text-secondary"></i> {{ $slot->teacher->name }}
                                                                        @else
                                                                            <i class="fas fa-users text-secondary"></i> {{ $slot->section->classLevel->name }} - {{ $slot->section->name }}
                                                                        @endif
                                                                    </small>

                                                                    {{-- Delete Button (Only show if managing a section) --}}
                                                                    @if($activeFilter === 'section')
                                                                        <form action="{{ route('timetable.destroy', $slot->id) }}" method="POST" class="mt-2 no-print">
                                                                            @csrf @method('DELETE')
                                                                            <button type="submit" class="btn btn-xs btn-outline-danger w-100" onclick="return confirm('Remove this slot?')">
